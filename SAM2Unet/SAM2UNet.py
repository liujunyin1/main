...
class UltraClsHead(nn.Module):
    """增强分类头"""

    def __init__(self, in_channels, hidden_dim, num_classes, extra_channels=[512]):
        super().__init__()
...
class SAM2UNet(nn.Module):
    def __init__(
        ...
        self.cls_decoder = UltraClsHead(
            in_channels=self.feat_dims[3],
            hidden_dim=256,
            num_classes=cls_class_num,
            extra_channels=[self.feat_dims[2]]
        )
    ...
    def _forward_task(self, x):
        ...
        cls_out = self.cls_decoder(x4, extra_feats=[x3])
