...
def create_debug_info_panel():
    ...
    # 1. 左侧 (角度) 布局 -> [修改] 改为 QGridLayout 以支持多列显示
    angle_layout_container = QWidget()
    angle_layout = QGridLayout(angle_layout_container)
    ...
    # --- 4. 添加 角度 标签 ([修改] 使用 Grid 布局添加 Value1/Value2) ---
    labels["debug_angle_header"] = QLabel("角度对称性分析:")
    labels["debug_angle_header"].setStyleSheet("font-weight: bold; margin-top: 5px;")
    angle_layout.addWidget(labels["debug_angle_header"], 0, 0, 1, 4)  # 跨4列

    # 添加表头
    angle_layout.addWidget(QLabel("角度对"), 1, 0)
    angle_layout.addWidget(QLabel("值1"), 1, 1)
    angle_layout.addWidget(QLabel("值2"), 1, 2)
    angle_layout.addWidget(QLabel("差值"), 1, 3)
    ...
    for i in range(4):
        ...
        # 设置字体
        l_val1.setFont(mono_font)
        l_val2.setFont(mono_font)
        l_diff.setFont(mono_font)

        # 存储到字典
        labels[f"debug_angle_name_{i + 1}"] = l_name
        labels[f"debug_angle_val1_{i + 1}"] = l_val1
        labels[f"debug_angle_val2_{i + 1}"] = l_val2
        labels[f"debug_angle_diff_{i + 1}"] = l_diff

        # 添加到网格 (行号 i+2)
        angle_layout.addWidget(l_name, i + 2, 0)
        angle_layout.addWidget(l_val1, i + 2, 1)
        angle_layout.addWidget(l_val2, i + 2, 2)
        angle_layout.addWidget(l_diff, i + 2, 3)
    ...
    angle_layout.addWidget(labels["debug_angle_avg_label"], 6, 0, 1, 2)
    angle_layout.addWidget(labels["debug_angle_avg_value"], 6, 2, 1, 2)

    # 增加弹簧以推高内容
    angle_layout.setRowStretch(7, 1)

    # 5. 组合主布局
    main_debug_layout.addWidget(angle_layout_container)
    main_debug_layout.addWidget(r_distance_container)
    main_debug_layout.setStretch(0, 3)  # [修改] 角度列加宽以容纳更多数据
    main_debug_layout.setStretch(1, 2)  # r-距离列
