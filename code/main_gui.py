...
class MainWindow(QMainWindow):
    ...
    def _display_current_image_result(self):
        ...
        if not self.show_analysis_view:
            ...
            for i in range(4):
                ...
                val_label = getattr(self, f"debug_angle_diff_{i + 1}")

                if i < len(pairs):
                    ang1_name, ang2_name = pairs[i]
                    name_label.setText(f"{ang1_name}/{ang2_name}:")
                    if ang1_name in angles and ang2_name in angles:
                        diff = math.fabs(angles[ang1_name] - angles[ang2_name])
                        val_label.setText(f"{diff:.4f}°")
                        diff_values.append(diff)
                    else:
                        val_label.setText("N/A")
                else:
    ...
    def _trigger_delete(self, ids_to_delete):
        ...
        worker = DataEditWorker(current_result, self.data_edit_signals, ids_to_remove=ids_to_delete)
        self.thread_pool.start(worker)
    ...
    def on_add_point_requested(self, scene_pos):
        ...
        worker = DataEditWorker(current_result, self.data_edit_signals, point_to_add=new_point_data)
        self.thread_pool.start(worker)
    ...
    def on_point_moved(self, point_id, scene_pos):
        ...
        worker = DataEditWorker(current_result, self.data_edit_signals, point_to_modify=modified_point_data)
        self.thread_pool.start(worker)
