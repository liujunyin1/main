...
def analyze_central_structure(inlier_data, image_height, image_width):
    ...
    # print(f"  信息: 中心筛选后，候选原子数量从 {len(inlier_data)} 筛选为 {len(analysis_candidates)}。") # <-- 已注释

    if len(analysis_candidates) < 9:
        # print("  信息: 筛选后内点数量不足（少于9个），跳过局部晶格分析。") # <-- 已注释
        return {}

    central_atom = min(analysis_candidates,
                       key=lambda p: math.dist((p["midpoint"]["x"], p["midpoint"]["y"]), (img_center_x, img_center_y)))
    ...
    # print(f"\n--- 局部晶格分析结果 ---\n晶格类型: {lattice_info.get('lattice_type', 'unknown')}") # <-- 已注释
    return lattice_info