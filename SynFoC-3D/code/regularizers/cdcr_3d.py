...
def cdcr_loss_3d(p_sam3d, p_vnet3d, w_edge=1.0, w_kl=1.0):
    ...
    """
    # 分布一致
    p_sam3d = p_sam3d.float()
    p_vnet3d = p_vnet3d.float()

    eps = 1e-4
    sam_safe = p_sam3d.clamp_min(eps)
    vnet_safe = p_vnet3d.clamp_min(eps)

    sam_safe = sam_safe / sam_safe.sum(dim=1, keepdim=True)
    vnet_safe = vnet_safe / vnet_safe.sum(dim=1, keepdim=True)

    sam_log = sam_safe.log()
    vnet_log = vnet_safe.log()

    kl_sv = F.kl_div(sam_log, vnet_safe, reduction='none')
    kl_vs = F.kl_div(vnet_log, sam_safe, reduction='none')
