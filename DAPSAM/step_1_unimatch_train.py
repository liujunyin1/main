...
def build_model(args, device):
    ...
    elif args.model == "desam":
        # register model
        model, img_embedding_size = sam_model_registry[args.vit_name](image_size=args.resize_target,
...
def build_optimizer(args, model):
    ...
    if opt_name == "desam":
        optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=args.base_lr,
...
def parse_args():
    ...
    p.add_argument("--model", type=str, default="desam", choices=["desam"])
    p.add_argument("--opt", type=str, default='desam', choices=[None, "desam"])

    p.add_argument("--ssl-ckpt", type=str, default='./pretrained_weights/echocare_encoder.pth')  # for echocare

    p.add_argument("--train-labeled-json", type=str,
                   default="/opt/data/private/fb/dataset/FETUS2026/Train/train_labeled.json")
    p.add_argument("--train-unlabeled-json", type=str,
                   default="/opt/data/private/fb/dataset/FETUS2026/Train/train_unlabeled.json")
    p.add_argument("--valid-labeled-json", type=str, default="/opt/data/private/fb/dataset/FETUS2026/Train/valid.json")

    p.add_argument("--train-epochs", type=int, default=300)
    p.add_argument("--batch-size", type=int, default=2)  # 32 for unet; 8 for echocare
    ...
    p.add_argument("--resize-target", type=int, default=384)
    p.add_argument("--save-path", type=str, default="./checkpoints_baseline_dapsam")
    ...
    p.add_argument('--vit_name', type=str,
                   default='vit_b', help='select one vit model')
    p.add_argument('--ckpt', type=str,
                   default='/opt/data/private/fb/MedSAM-main/work_dir/SAM/sam_vit_b_01ec64.pth',
                   help='Pretrained checkpoint')
    p.add_argument('--module', type=str, default='sam_lora_image_encoder')
    p.add_argument('--rank', type=int, default=4, help='Rank for LoRA adaptation')
    ...
    # p.add_argument("--pseudo-tau-pos", type=float, default=0.85)
    p.add_argument("--pseudo-tau-pos", type=float, default=0.8)
    # p.add_argument("--pseudo-tau-neg", type=float, default=0.85)
...
def maybe_resume(model, optimizer, scaler, ckpt_path: str, logger: logging.Logger):
    ...
    logger.info(
        f"[Resume] epoch={start_epoch}, best_score={best_score:.4f}, best_epoch={best_epoch}, global_step={global_step}")
...
def teacher_pseudo(
    ...
    model.eval()
    outputs = model(image_u_w_mix, False, image_size)
    pred_u_w_mix, cls_u_w_mix = outputs["low_res_logits"], outputs["cls_logits"]
...
def train_one_epoch(
    ...
    for i, (
        ...
        with torch.cuda.amp.autocast(enabled=use_amp, dtype=amp_dtype):
            preds_outputs = model(torch.cat((image_x, image_u_w)), False, args.resize_target, need_fp=True)
            (preds, preds_fp) = preds_outputs['low_res_logits'][0], preds_outputs['low_res_logits'][1]
            (preds_class, preds_class_fp) = preds_outputs['cls_logits'][0], preds_outputs['cls_logits'][1]
            ...
            pred_u_s_outputs = model(torch.cat((image_u_s1, image_u_s2)), False, args.resize_target)
            pred_u_s_outs = pred_u_s_outputs['low_res_logits']
            pred_u_s1, pred_u_s2 = pred_u_s_outs.chunk(2)

            pred_u_s1_mix_outputs = model(image_u_s1_mix, False, args.resize_target)
            pred_u_s1_mix, pred_class_u_s1_mix = pred_u_s1_mix_outputs['low_res_logits'], pred_u_s1_mix_outputs[
                'cls_logits']
            pred_u_s2_mix_outputs = model(image_u_s2_mix, False, args.resize_target)
            pred_u_s2_mix, pred_class_u_s2_mix = pred_u_s2_mix_outputs['low_res_logits'], pred_u_s2_mix_outputs[
                'cls_logits']

            pred_class_u_w_mix_outputs = model(image_u_w_mix, False, args.resize_target, True)
            (pred_class_u_w_mix, pred_class_u_w_mix_fp) = pred_class_u_w_mix_outputs['cls_logits'][0], \
            pred_class_u_w_mix_outputs['cls_logits'][1]
        ...
        if use_amp and scaler is not None and amp_dtype == torch.float16:
            ...
            # === 新增：梯度裁剪 ===
            # scaler.unscale_(optimizer)  # Step 1: 先 unscale 梯度
            # torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)  # Step 2: 裁剪梯度，max_norm通常设为1.0或5.0
            # ====================
...
def validate(args, model, device, valid_loader, allowed_seg_mat, cls_allowed):
    ...
    for image, view, mask, class_label in valid_loader:
        ...
        pred_out = model(image_rs, False, args.resize_target)
        pred_mask_logits_rs, pred_class_out = pred_out['masks'], pred_out['cls_logits']
        pred_mask_logits = F.interpolate(
            pred_mask_logits_rs, (h, w),
            mode="bilinear", align_corners=False
        )

        seg_logits = apply_view_mask_logits(pred_mask_logits, v,
                                            allowed_seg_mat) if use_hard_view_mask else pred_mask_logits
...
def main():
    ...
    allowed_cls_mat = build_allowed_mat(device, cls_allowed, num_views=args.view_num_classes,
    ...
    pos_weight = compute_pos_weight_from_loader(train_loader_l, allowed_cls_mat, args.cls_num_classes, device).to(
        device)
