...
class SAM2Base(torch.nn.Module):
    ...
    def _forward_sam_heads(
        ...
        return_refined_features: bool = False,
    ):
        """
        ...
        if dense_embeddings.shape[-2:] != backbone_features.shape[-2:]:
            dense_embeddings = F.interpolate(
                dense_embeddings,
                size=backbone_features.shape[-2:],
                mode="bilinear",
                align_corners=False,
            )
        image_pe = self.sam_prompt_encoder.get_dense_pe()
        if image_pe.shape[-2:] != backbone_features.shape[-2:]:
        ...
        if return_refined_features:
            (
                ...
                object_score_logits,
                refined_features,
            ) = self.sam_mask_decoder.predict_masks_with_features(
        ...
        if return_refined_features:
            return outputs + (refined_features,)
    ...
    def track_step(
        ...
        _, _, _, low_res_masks, high_res_masks, obj_ptr, _, *_ = sam_outputs
