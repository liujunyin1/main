...
FIXED_DESAM_OVERLOCK_CKPT = (
    r"D:\Projects\FETUS - v2\checkpoints\120(desam+overlock).pth"
)
...
def load_checkpoint_strict(
    model: torch.nn.Module, ckpt_path: str, device: torch.device, logger: logging.Logger
):
...
def load_seg_allowed(
    seg_allowed_arg: Optional[str], default: Dict[int, List[int]]
) -> Dict[int, List[int]]:
    ...
    if not isinstance(raw, dict):
        raise ValueError(
            "--seg-allowed must be a JSON object: {view_id: [class_ids...]}"
        )
...
def build_allowed_mat(
    device: torch.device,
    seg_allowed: Dict[int, List[int]],
    num_views: int,
    num_classes: int,
) -> torch.Tensor:
    ...
    for v in range(num_views):
        ...
        for c in cls_ids:
            if c < 0 or c >= num_classes:
                raise ValueError(
                    f"seg_allowed[{v}] contains invalid class id {c} for num_classes={num_classes}"
                )
        mat[v, cls_ids] = True
    return mat
...
def apply_view_mask_probs(
    probs: torch.Tensor, view_ids: torch.Tensor, allowed_mat: torch.Tensor
) -> torch.Tensor:
    invalid = ~allowed_mat[view_ids]
    return probs.masked_fill(invalid, 0.0)


def parse_thr_per_class(
    thr_str: str, K: int
    ...
    if len(parts) != K:
        raise ValueError(
            f"--cls-thr-per-class expects {K} values, got {len(parts)}: {thr_str}"
        )
...
def prob_to_binary(
    prob: np.ndarray[Any, np.dtype[np.float32]],
    thr_global: float,
    thr_per_class: Optional[np.ndarray[Any, np.dtype[np.float32]]],
...
def save_pred_h5(
    save_path: str,
    pred_mask_hw: np.ndarray[Any, np.dtype[np.uint8]],
    pred_label_k: np.ndarray[Any, np.dtype[np.uint8]],
):
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    with h5py.File(save_path, "w") as f:
        f.create_dataset("mask", data=pred_mask_hw.astype(np.uint8), compression="gzip")
        f.create_dataset(
            "label", data=pred_label_k.astype(np.uint8), compression="gzip"
        )
...
def run_inference(
    ...
    device: torch.device,
    allowed_seg_mat: torch.Tensor,
    allowed_cls_mat: torch.Tensor,
    args: Any,
    ...
    for batch in tqdm(loader, total=len(loader), desc="Infer"):
        ...
        need_oracle_view = (args.mask_mode == "oracle") or (
            args.cls_mask_mode == "oracle"
        )
        ...
        if need_oracle_view:
            ...
            if view_oracle.numel() != B:
                raise RuntimeError(
                    f"view_oracle size mismatch: got {view_oracle.numel()}, expected {B}"
                )
            if (view_oracle.min() < 0) or (view_oracle.max() >= args.view_num_classes):
                raise ValueError(
                    f"Invalid view id in batch: {view_oracle.detach().cpu().tolist()}"
                )
        ...
        if args.mask_mode == "oracle":
            masked_logits = apply_view_mask_logits(
                pred_mask_logits, view_oracle, allowed_seg_mat
            )
        ...
        if args.cls_mask_mode == "oracle":
            pred_prob = apply_view_mask_probs(pred_prob, view_oracle, allowed_cls_mat)
        elif args.cls_mask_mode != "none":
            raise ValueError(f"Unknown cls_mask_mode: {args.cls_mask_mode}")
        ...
        for b in range(B):
            ...
            if args.cls_mask_mode == "oracle":
                pl = pl * allowed_cls_mat[view_oracle[b]].detach().cpu().numpy().astype(
                    np.uint8
                )
...
def parse_args():
    ...
    p.add_argument(
        ...
        default="oracle",
        choices=["oracle", "none"],
        help="oracle: mask classification probs with dataset-provided view; none: no classification masking",
    )
    p.add_argument(
        "--cls-allowed",
        type=str,
...
def main():
    ...
    logger.info(f"Fixed checkpoint: {FIXED_DESAM_OVERLOCK_CKPT}")

    seg_allowed = load_seg_allowed(args.seg_allowed, DEFAULT_SEG_ALLOWED)
    cls_allowed = load_seg_allowed(args.cls_allowed, DEFAULT_CLS_ALLOWED)
    allowed_seg_mat = build_allowed_mat(
        device, seg_allowed, args.view_num_classes, args.seg_num_classes
    )
    allowed_cls_mat = build_allowed_mat(
        device, cls_allowed, args.view_num_classes, args.cls_num_classes
    )

    model = SAM2UNet(
        checkpoint_path=None,
        overlock_weights_path=None,
        in_chns=1,  # 新增，保持兼容
    ...
    load_checkpoint_strict(model, FIXED_DESAM_OVERLOCK_CKPT, device, logger)
    ...
    run_inference(model, loader, device, allowed_seg_mat, allowed_cls_mat, args, logger)
