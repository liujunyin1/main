...
def load_allowed_mapping(
    value: Optional[str], default: Dict[int, List[int]]
) -> Dict[int, List[int]]:
    ...
    if not isinstance(raw, dict):
        raise ValueError(
            "Allowed mapping must be a JSON object: {view_id: [class_ids...]}"
        )
...
def load_loss_weights(
    value: Optional[str], default: Dict[str, float]
) -> Dict[str, float]:
    raw = _load_json_arg(value)
    if raw is None:
        return default
    ...
    for k, v in raw.items():
        if k not in out:
            raise ValueError(
                f"Unknown loss weight key: {k}. Allowed keys: {sorted(out.keys())}"
            )
        out[k] = float(v)
    return out
...
def build_model(args, device):
    if args.model == "unet":
        ...
        # 新增SAM2UNet支持
        if not hasattr(args, "hiera_path") or args.hiera_path is None:
            raise ValueError("SAM2UNet requires --hiera-path argument")
        model = SAM2UNet(
            checkpoint_path=args.hiera_path,
            overlock_weights_path=args.overlock_weights_path,
            device=device,
            in_chns=1,  # 新增，保持兼容
            seg_class_num=args.seg_num_classes,  # 改为seg_class_num
            cls_class_num=args.cls_num_classes,  # 改为cls_class_num
            img_size=352,
            channel_mode="replicate",
            seg_mode=args.seg_mode,
            use_overlock=args.use_overlock,
        )
    else:
...
def build_optimizer(args, model):
    ...
    if opt_name == "sam2unet":
        params = model.get_trainable_params()

        adapter_params = params["adapter"]
        head_params = params["head"]

        print(f"adapter params: {sum(p.numel() for p in adapter_params) / 1e6:.4f}M")
        print(f"head params: {sum(p.numel() for p in head_params) / 1e6:.4f}M")

        base_adapter_lr = 1e-4
        base_head_lr = 1e-3

        optimizer = torch.optim.AdamW(
            [
                {"params": adapter_params, "lr": base_adapter_lr},
                {"params": head_params, "lr": base_head_lr},
            ],
            weight_decay=0.01,
        )

        base_lrs = [base_adapter_lr, base_head_lr]
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=str,
        default="sam2unet",
        choices=["unet", "echocare", "sam2unet"],
    )
    p.add_argument(
        "--opt",
        type=str,
        default="sam2unet",
        choices=[None, "unet", "echocare", "sam2unet"],
    )

    p.add_argument(
        "--ssl-ckpt", type=str, default="./pretrained_weights/echocare_encoder.pth"
    )  # for echocare
    ...
    p.add_argument(
        "--hiera-path",
        type=str,
        default="../v1/checkpoints/sam2_hiera_tiny.pt",
        help="Path to SAM2 pretrained Hiera checkpoint (required for sam2unet)",
    )
    p.add_argument(
        "--overlock-weights-path",
        type=str,
        default="../v1/checkpoints/overlock_xt_in1k_224.pth",
        help="Optional path to migrated OverLoCK CNN branch weights for sam2unet",
    )
    p.add_argument(
        "--seg-mode",
        type=str,
        default="hybrid_x4",
        choices=["legacy_concat", "hybrid_x4"],
        help="Segmentation path for SAM2UNet",
    )
    p.add_argument(
        "--no-overlock",
        action="store_false",
        dest="use_overlock",
        help="Disable the optional OverLoCK auxiliary branch in SAM2UNet",
    )
    p.set_defaults(use_overlock=True)
    p.add_argument(
        "--train-labeled-json",
        type=str,
        default="/opt/data/private/SH/dataset/FETUS26/train_labeled.json",
    )
    p.add_argument(
        "--train-unlabeled-json",
        type=str,
        default="/opt/data/private/SH/dataset/FETUS26/train_unlabeled.json",
    )
    p.add_argument(
        "--valid-labeled-json",
        type=str,
        default="/opt/data/private/fb/dataset/FETUS2026/Train/valid.json",
    )
    ...
    p.add_argument("--resize-target", type=int, default=352)
    p.add_argument(
        "--save-path", type=str, default="./checkpoints_baseline_sam2unet_tiny"
    )
...
def build_seg_allowed_mat(
    device, seg_allowed: Dict[int, List[int]], num_views: int, num_classes: int
) -> torch.Tensor:
...
def train_one_epoch(
    ...
    for i, (
        ...
        with torch.cuda.amp.autocast(enabled=use_amp, dtype=amp_dtype):
            ...
            (_, _), (pred_class_u_w_mix, pred_class_u_w_mix_fp) = model(
                image_u_w_mix, True
            )
        ...
        if use_hard_view_mask:
            ...
            pred_u_s1_mix = apply_view_mask_logits(
                pred_u_s1_mix, view_u_mix, allowed_seg_mat
            )
            pred_u_s2_mix = apply_view_mask_logits(
                pred_u_s2_mix, view_u_mix, allowed_seg_mat
            )
...
@torch.no_grad()
def validate(
    args, model, device, valid_loader, allowed_seg_mat, cls_allowed
) -> Dict[str, Any]:
    model.eval()
    ...
    for image, view, mask, class_label in valid_loader:
        ...
        pred_mask_logits = F.interpolate(
            pred_mask_logits_rs, (h, w), mode="bilinear", align_corners=False
        )

        seg_logits = (
            apply_view_mask_logits(pred_mask_logits, v, allowed_seg_mat)
            if use_hard_view_mask
            else pred_mask_logits
        )
    ...
    y_true_all = (
        np.stack(y_true_all, axis=0)
        if len(y_true_all)
        else np.zeros((0, K), dtype=np.float32)
    )
    y_prob_all = (
        np.stack(y_prob_all, axis=0)
        if len(y_prob_all)
        else np.zeros((0, K), dtype=np.float32)
    )
    views_all = (
        np.array(views_all, dtype=np.int32)
        if len(views_all)
...
def main():
    ...
    logger.info(
        f"pseudo_tau_pos={args.pseudo_tau_pos}, pseudo_tau_neg={args.pseudo_tau_neg}"
    )
    ...
    allowed_seg_mat = build_seg_allowed_mat(
        device, seg_allowed, args.view_num_classes, args.seg_num_classes
    )
    allowed_cls_mat = build_allowed_mat(
        device,
        cls_allowed,
        num_views=args.view_num_classes,
        num_classes=args.cls_num_classes,
    )

    db_train_u = FETUSSemiDataset(
    ...
    valid_loader = DataLoader(
        db_valid, batch_size=1, pin_memory=True, num_workers=1, drop_last=False
    )

    pos_weight = compute_pos_weight_from_loader(
        train_loader_l, allowed_cls_mat, args.cls_num_classes, device
    ).to(device)
    logger.info(
        f"pos_weight (masked by cls_allowed): {pos_weight.detach().cpu().numpy()}"
    )
    ...
    for epoch in range(start_epoch + 1, args.train_epochs):
        ...
        log_val_perclass_tb(
            writer, val["dice_class_view_masked"], val["nsd_class_view_masked"], epoch
        )
